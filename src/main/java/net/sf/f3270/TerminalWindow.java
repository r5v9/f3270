...
public class TerminalWindow {
    ...
    public void update(final String command, final String returned, final Param... params) {
        updateTerminal();
    ...
    private void updateDebug(final String command, final String returned, final Param... params) {
        ...
        SwingUtilities.invokeLater(new Runnable() {
            public void run() {
                textPaneDebug.scrollRectToVisible(new Rectangle(0, textPaneDebug.getHeight() * 2, 1, 1));
            }
        });
    }
    ...
    private JScrollPane buildTextPaneDebug(final Font monospacedFont, final Font sansFont) {
        ...
        textPaneDebugScroller.setPreferredSize(new Dimension(textPane3270.getWidth(), 3 + 10 * fontMetricsSans
                .getHeight()));
    ...
    private JScrollPane buildFieldsTablePanel() {
        fieldsTable = new JTable(new AbstractTableModel() {
            ...
            public Object getValueAt(final int rowIndex, final int columnIndex) {
                ...
                final Field f = s3270.getScreen().getFields().get(rowIndex);
                if (columnIndex == 1) {
                    return ((f instanceof InputField) ? "in" : "out")
                            + (((f instanceof InputField) && ((InputField) f).isChanged()) ? " *" : "");
                }
                if (columnIndex == 2) {
    ...
    private Style createStyle(final Color foregroundColor, final Color backgrondColor, final boolean isItalic) {
        final String key = String.format("%d-%d-%d %d-%d-%d %s", foregroundColor.getRed(), foregroundColor.getGreen(),
                foregroundColor.getBlue(), backgrondColor.getRed(), backgrondColor.getGreen(),
                backgrondColor.getBlue(), isItalic);
